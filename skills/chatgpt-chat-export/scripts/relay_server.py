#!/usr/bin/env python3
"""
Tiny local HTTP relay for MCP-driven chatgpt.com exports (fallback transport).

When the export is driven via the claude-in-chrome MCP extension, the MCP's
javascript_tool return values are truncated (~1000 chars) and filtered
(JWT-shaped, base64, query-string and long-uniform payloads get replaced with
[BLOCKED: ...]) — so neither the conversation JSON nor attachment bytes can be
returned from JS directly.

One workaround is this relay: run it locally, then have the browser POST the
fetched conversation JSON and each attachment blob to it. The relay writes
files straight into the export directory.

CAVEAT: uBlock Origin (Lite)'s "Block outsider intrusion into LAN" list
blocks page requests from chatgpt.com to 127.0.0.1 — fetch throws
"Failed to fetch". If that happens, use the clipboard transport documented in
SKILL.md instead (it needs no network), or whitelist chatgpt.com in uBlock.
...
def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="Local HTTP relay for MCP-driven chatgpt.com exports")
